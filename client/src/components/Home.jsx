...
const Home = () => {
  ...
  const fetchLoans = async () => {
    try {
      const loanData = await axios.get(
        `${import.meta.env.VITE_BE_URL}/api/v1/loans`,
        {
          headers: {
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.patch(
        `${import.meta.env.VITE_BE_URL}/api/v1/loans/update-status/`,
        { id, status },
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const updatePayment = async (loanId, installmentId) => {
    try {
      await axios.patch(
        `${import.meta.env.VITE_BE_URL}/api/v1/loans/repay/`,
              ...
              {loans.map((loan, idx) => (
                <React.Fragment key={idx}>
                  ...
                  {showDetails === loan._id && (
                    <tr>
                      <td colSpan={user.user_type === "admin" ? 8 : 5}>
                        <div>
                          ...
                          <h2>Remaining Amount (w/ interest): {loan.remainingAmount}</h2>
                        </div>
                      </td>
                    </tr>
                  )}
